module.exports = class Restrouter {
  ...
  constructor(expressApp,_class,populate,entityName,access){
    ...
    // update 2017-05-03 by WeeHorse:
    // the rest route:
    //   specify it in the _class with a static getter, like:
    //   static get restRoute(){return 'whateverroute';}
    //   - but if you dont, automatic lowercase from _class name will be used
    ...
    //   specify it in the _class with a static getter, like:
    //   static get restRoute(){return 'whateverroute';}
    //   or automatic lowercase from _class name will be used
    this.baseRoute = this._class.restRoute ||  '/rest/' + className.toLowerCase() + '/';
  ...
  json(res,err,response){
    ...
    if(!(err || response)){
      res.json({'fatal':'Something is really wrong!'});
      return;
    }
    // send the response
